...
async fn run_tests() -> String {
    ...
    println!("Standard loading took {:?}", now.elapsed());
    ...
    let (
        ((mut graph_walk1, mut graph_walk2), mut graph_walk3),
        ((mut graph_routes1, mut graph_routes2), (mut graph_routes3, _node_values_2d)),
    ) = rayon::join(
        || {
            rayon::join(
                || {
                    rayon::join(
                        || {
                            deserialize_bincoded_file::<Vec<NodeWalk>>(&format!(
                                "graph_pt_walk_chunk_1"
                            ))
                        },
                        || {
                            deserialize_bincoded_file::<Vec<NodeWalk>>(&format!(
                                "graph_pt_walk_chunk_2"
                            ))
                        },
                    )
                },
        ...
        || {
            rayon::join(
                || {
                    rayon::join(
                        || {
                            deserialize_bincoded_file::<Vec<NodeRoute>>(&format!(
                                "graph_pt_routes_chunk_1"
                            ))
                        },
                        || {
                            deserialize_bincoded_file::<Vec<NodeRoute>>(&format!(
                                "graph_pt_routes_chunk_2"
                            ))
                        },
                    )
                },
                || {
                    rayon::join(
                        || {
                            deserialize_bincoded_file::<Vec<NodeRoute>>(&format!(
                                "graph_pt_routes_chunk_3"
                            ))
                        },
                        || {
                            deserialize_bincoded_file::<Vec<Vec<SubpurposeScore>>>(&format!(
                                "sparse_node_values_6am_{year}_2d"
                            ))
                        },
                    )
                },
    ...
    let (
        ((mut graph_walk1, mut graph_walk2), (mut graph_walk3, mut graph_walk4)),
        ((mut graph_routes1, mut graph_routes2), (mut graph_routes3, mut graph_routes4)),
    ) = rayon::join(
        || {
            rayon::join(
                || {
                ...
                || {
                    rayon::join(
                        || {
                            deserialize_bincoded_file::<Vec<NodeWalk>>(&format!(
                                "graph_pt_walk_chunk_3"
                            ))
                        },
                        || {
                            deserialize_bincoded_file::<Vec<NodeWalk>>(&format!(
                                "graph_pt_walk_chunk_4"
                            ))
                        },
                    )
                },
    ...
    let _node_values_2d: Vec<Vec<SubpurposeScore>> =
